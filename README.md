...
- 추세(이동평균)와 변동성 지표로 공격/수비 전환:
  ...
  - 그 외: 방어적으로 QQQM
- 체결 가정: 매매 신호/추천이 발생한 다음 거래일 시초가로 전액 전환하며, 매수는 슬리피지만큼 높은 가격(+buy_pct), 매도는 슬리피지만큼 낮은 가격(-sell_pct)으로 보수적으로 체결가를 추정합니다. 거래비용/세금은 별도 반영하지 않습니다. 상태는 BUY/HOLD/SELL/WAIT로 기록.

## 추천(Recommend) 흐름
- `python recommend.py` 실행 시 “마지막 거래일 종가 기준” 신호를 생성합니다. 장 마감 이후 실행해 최신 신호를 확인하고, 다음 거래일 시초가에 맞춰 매수/매도하는 흐름이 백테스트 가정과 가장 유사합니다.
- 백테스트와의 차이: 실제 체결가는 슬리피지/스프레드/수수료/세금, 데이터 지연, 휴장/조기폐장 등으로 달라질 수 있습니다. 장 마감 직후에 추천을 갱신하고, 다음 거래일 시초가 체결을 따르는 방식이 백테스트와 가장 근접합니다.
