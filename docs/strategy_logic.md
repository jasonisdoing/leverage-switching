# 전략 로직 (Strategy Logic)

## 1. 핵심 전략: 2-Tier Hysteresis Switching
이 프로젝트는 단순한 모멘텀 전략이 아닌, **히스테리시스(Hysteresis)** 개념을 도입한 스위칭 전략을 사용합니다. 이는 잦은 매매 신호 번복(Whipsaw)을 방지하고 추세의 안정성을 확보하기 위함입니다.

### 작동 원리
시그널 자산인 **기초 지수 ETF(QQQ, KODEX 코스피 등)**의 **고점 대비 하락률(Drawdown)**을 기준으로 공격 자산과 방어 자산을 교체합니다.

- **공격 자산 (Offense)**: `TQQQ` 또는 `KODEX 레버리지`
- **방어 자산 (Defense)**: `GDX`, `PLUS 고배당주` 등 (최적 자산 선정)

### 이중 임계값 (Dual Thresholds)
매수와 매도 기준을 다르게 설정하여 시장의 노이즈를 걸러냅니다.

1.  **매도 기준 (`drawdown_sell_cutoff`)**:
    - 공격 자산을 보유 중일 때, 시그널 하락률이 이 기준보다 **더 낮아지면(악화되면)** 방어 자산으로 교체합니다.
    - 예: `sell_cutoff = 0.4%` → 하락률이 -0.4% 밑으로 내려가면 매도.

2.  **매수 기준 (`drawdown_buy_cutoff`)**:
    - 방어 자산을 보유 중일 때, 시그널 하락률이 이 기준보다 **더 높아지면(회복되면)** 공격 자산으로 교체합니다.
    - 예: `buy_cutoff = 0.3%` → 하락률이 -0.3% 위로 올라오면 매수.

> **핵심**: 항상 `buy_cutoff < sell_cutoff` 조건을 유지하여, "확실히 떨어지면 팔고, 확실히 오르면 산다"는 원칙을 지킵니다. 그 사이 구간(Dead Zone)에서는 포지션을 변경하지 않고 유지(Hold)합니다.

### 매매 실행 기준
시그널은 당일 종가 기준으로 확정하고, 실제 포지션 변경은 다음 거래일 시초가에 실행하는 것으로 계산합니다.

> **장중 추천(recommend) 동작**: 장중에는 오늘의 미완성 봉을 신호 계산에서 제외하고(`drop_today`),
> **마지막으로 닫힌 거래일 종가로 확정된 보유 종목**을 그대로 표시합니다. 장중에는 포지션을 바꾸지 않고
> "장 마감 시까지 변동될 수 있습니다 / 장 마감 후 최종 확정 알림을 기다려주세요" 안내만 보냅니다(@channel 없음).
> 단, **'현재가'만 오늘 실시간값으로 표시**(정보용, `(장중 실시간)` 표기)하고 '일간'은 (오늘 실시간 ÷ 어제 확정 종가)로 계산합니다.
> 신호·포지션·평단·익절 판단은 어제 종가 확정값을 유지합니다.
> 장 마감 후 실행에서 오늘 종가로 확정하며, 직전 확정 대비 바뀌면 "다음 거래일 시초가 매매" 알림과 `@channel`을 보냅니다.

## 2. 튜닝 알고리즘 (Tuning Algorithm)

### 전수 조사 (Exhaustive Search)
최적의 파라미터를 찾기 위해 가능한 모든 조합을 테스트합니다.

- **튜닝 대상 파라미터**:
    - `drawdown_buy_cutoff`: 0.1% ~ 3.0% (0.1% 단위)
    - `drawdown_sell_cutoff`: 0.1% ~ 3.0% (0.1% 단위)
    - `defense`: GDX, SCHD, PLUS 고배당주 등 종목 리스트

### 데이터 검증 (Data Validation)
튜닝 시작 전, 모든 방어 자산 후보군에 대해 **백테스트 시작일(`start_bound`)** 기준 데이터 가용성을 확인합니다. 데이터가 부족한 종목이 있으면 튜닝을 중단하고 해결 방법을 안내합니다.

### 최적화 기준
- **CAGR (Compound Annual Growth Rate)**: 연평균 수익률이 가장 높은 조합을 최우선으로 선택합니다.

### 데이터 기간
- 설정된 `months_range`(예: 12개월)를 사용하며, 신호 계산을 위한 **12개월 영업일(252일)**의 Warmup 기간을 추가로 로드합니다.

## 3. 시그널 계산 상세

### Drawdown 계산
```python
Peak = cummax(Price)
Drawdown = (Price / Peak) - 1.0
```

### 포지션 결정 (`pick_target`)
```python
if prev_target == offense:
    if current_dd < sell_cut:
        return defense  # 매도
    return offense      # 유지
else:
    if current_dd > buy_cut:
        return offense  # 매수
    return defense      # 유지
```

## 4. 추천과 백테스트의 통합

> **중요**: `recommend.py`는 독립적인 추천 로직이 **없습니다**.

**설계 원칙**: 백테스트 결과의 마지막 날 = 오늘의 추천

이렇게 하면:
- ✅ 백테스트와 추천이 항상 **100% 일관된 결과**를 보장
- ✅ 로직 변경 시 **한 곳만 수정**하면 됨
- ✅ 유지보수 부담 감소

## 5. 무한매수법 (buy) — 1단계 단순화 버전

스위칭과 별개로, 단일 대상 종목을 분할 매수하는 전략입니다. 엔진은 `logic/infinite_buy/runner.py`.

### 작동 원리
- 원금을 `divisions`(분할 수)로 나눠 **매 거래일 1/divisions 씩 시초가로 매수**한다.
- 매수할 때마다 평단가(체결가 기준, 슬리피지 포함)를 갱신한다.
- 보유 전량 평가가 **평단 × (1 + take_profit_pct/100)** 에 도달하면 그날 시초가에 **전량 익절**하고,
  **다음 거래일부터 새 사이클(1회차)** 을 다시 시작한다.
- `divisions` 회까지 매수하면(원금 소진) 추가 매수를 멈추고 **보유 유지(존버)** 하며 익절만 대기한다.
- 미투입 현금은 수익률 0으로 보유한다. CAGR/MDD 등은 원금 전체(현금+주식) 기준으로 산출한다.

### 튜닝
- `divisions`(분할 수) × `take_profit_pct`(익절률) 격자를 전수 탐색하여 CAGR 최대 조합을 선택한다.

### 특성
- 강한 상승장에서는 +익절률마다 현금화하므로 단순보유 대비 업사이드를 일부 포기한다.
- 하락장에서는 분할 매수로 평단을 낮추며 버틴다.

> 1단계는 일봉(시초가/종가) 기반의 단순화 버전이다. LOC 절반/지정가 절반, T값 기반 강약 조절(2세대),
> VR(밸류 리밸런싱) 등은 후속 단계로 확장 예정.
